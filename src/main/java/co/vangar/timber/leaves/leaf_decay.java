package co.vangar.timber.leaves;

import co.vangar.timber.Timber;
import org.bukkit.*;
...
public class leaf_decay implements Listener {

    /**
     *All credit for leaf decay goes to
     * StarTux FastLeafDecay last updated 1.13
     * https://www.spigotmc.org/resources/fastleafdecay.60237/
    */
        ...
        for (BlockFace neighborFace: NEIGHBORS) {
            ...
            if (oneByOne) {
                if (scheduledBlocks.isEmpty()) {
                    getServer().getScheduler().runTaskLater(plugin, this::decayOne, delay);
                }
                scheduledBlocks.add(block);
            } else {
                getServer().getScheduler().runTaskLater(plugin, () -> decay(block), delay);
            }
            scheduledBlocks.add(block);
        }
    }
